  ...
  function shiftDown(rowId) {
    ...
    if (row.nextElementSibling) {
      ...
      table.insertBefore(
        ...
        row.previousElementSibling.previousElementSibling
      );
      return true;
    }
    return false;
  ...
  function showSlide(index) {
    slides.forEach((slide, idx) => {
      ...
      slide.style.transform =
        idx === index ? "rotateY(0deg)" : "rotateY(90deg)";
    });
  }
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const star =
    "M12 20.1l5.82 3.682c1.066.675 2.37-.322 2.09-1.584l-1.543-6.926 5.146-4.667c.94-.85.435-2.465-.799-2.567l-6.773-.602L13.29.89a1.38 1.38 0 0 0-2.581 0l-2.65 6.53-6.774.602C.052 8.126-.453 9.74.486 10.59l5.147 4.666-1.542 6.926c-.28 1.262 1.023 2.26 2.09 1.585L12 20.099z";
  const unstar =
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  let removebuttons = document.querySelectorAll(
    'button[type="RemoveFromWatchList"]'
  );
  // const removebuttons = [];
  ...
  function createMovieElement(index) {
    ...
    var starIconSvg = document.createElementNS(
    ...
    var starIconPath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    ...
    var starBorderSvg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    var starBorderPath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    starBorderPath.setAttribute(
      "d",
      "M22.724 8.217l-6.786-.587-2.65-6.22c-.477-1.133-2.103-1.133-2.58 0l-2.65 6.234-6.772.573c-1.234.098-1.739 1.636-.8 2.446l5.146 4.446-1.542 6.598c-.28 1.202 1.023 2.153 2.09 1.51l5.818-3.495 5.819 3.509c1.065.643 2.37-.308 2.089-1.51l-1.542-6.612 5.145-4.446c.94-.81.45-2.348-.785-2.446zm-10.726 8.89l-5.272 3.174 1.402-5.983-4.655-4.026 6.141-.531 2.384-5.634 2.398 5.648 6.14.531-4.654 4.026 1.402 5.983-5.286-3.187z"
    );
    ...
    var addIconSvg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    var addIconPath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    addIconPath.setAttribute(
      "d",
      "M5 11h14c.55 0 1 -.45 1 -1s-.45 -1 -1 -1H5c-.55 0 -1 .45 -1 1s.45 1 1 1z"
    );
    ...
    var playArrowSvg = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    var playArrowPath = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "path"
    );
    playArrowPath.setAttribute(
      "d",
    ...
    for (let i = 0; i < removebuttons.length; i++) {
      removebuttons[i].addEventListener("click", function () {
        document
          ...
          .removeChild(removebuttons[i].parentElement.parentElement);
        buttons.forEach((element) => {
          if (
            element.parentElement.previousElementSibling.innerText ===
            removebuttons[i].parentElement.previousElementSibling.innerText
          ) {
